...
class MarineWeatherCurrentSensor(SensorEntity):
    ...
    async def async_added_to_hass(self):
        ...
        _LOGGER.debug(f"MarineWeatherCurrentSensor for {self._name} added to Home Assistant.")
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    Create sensors based on the configuration provided in YAML.
    """
    sensors = []

    # Loop through the configuration and create sensors for each location
    ...
    for location in locations:
        ...
        _LOGGER.debug(f"Setting up MarineWeatherCurrentSensor for {name}")
